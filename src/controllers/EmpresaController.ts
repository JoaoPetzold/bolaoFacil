...
export default {
    async show(req: Request, res: Response){
        const EmpresaRep = getRepository(Empresa)
        const empresas = await EmpresaRep.find();
    ...
    async showOne(req: Request, res: Response){
        ...
        const EmpresaRep = getRepository(Empresa)
        const empresa = await EmpresaRep.findOne({CNPJ});
        return res.json({EMPRESA: empresa})
    },

    async update(req: Request, res: Response){
        const {CNPJ} = req.params;
        const {NOME, DATA_ENVIO} = req.body;
        const data_atual = moment(DATA_ENVIO).format('YYYY/MM/DD hh:mm:ss')
        console.log("CORPO: " + JSON.stringify(data_atual))
        
        
        const EmpresaRep = getRepository(Empresa);
        const empresas = await EmpresaRep.findOne({CNPJ});

        if(empresas){
            await EmpresaRep.update(CNPJ, req.body)
            console.log("ALTERADO: " + JSON.stringify(NOME))
    ...
    async create(req: Request, res: Response){
        ...
        const empresas = await EmpresaRep.findOne({CNPJ});
        console.log("CNPJ DA EMPRESA: " + CNPJ)
        console.log("DATA DA EMPRESA ATUAL: " + moment(data).format('YYYY/MM/DD hh:mm:ss'))
        
        if(empresas){
            console.log("DATA DA EMPRESA DO EXE: " + moment(empresas.DATA_EXE).format('YYYY/MM/DD'))
            await EmpresaRep.update(CNPJ, req.body)
            console.log("ALTERADO COM SUCESSO: " + JSON.stringify(req.body))
            return res.status(200).json({MENSAGEM: 'CNPJ EXISTENTE, A ALTERAÇÃO FOI FEITA COM SUCESSO'})
        }else{
            const empresa = EmpresaRep.create({
                NOME,
                CNPJ,
                DATA_EXE,
                DATA_ENVIO
            });
            console.log("CRIADO COM SUCESSO: " + JSON.stringify(req.body))
            await EmpresaRep.save(empresa); 
            return res.status(201).json([{EMPRESA: empresa, MENSAGEM: 'CRIADO COM SUCESSO'}]);
        }
